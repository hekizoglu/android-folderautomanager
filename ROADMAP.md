...
**Durum:** ✅ Tamamlandı (Döngü 286) — `AppContextMenu.kt` callback signature hatalı idi: `onChangeCategory: () -> Unit` yerine `(AppInfo) -> Unit` olmalıydı. Satır 67 ve 222 düzeltildi. `FolderScreen.kt` eski yorum kaldırılıp callback alındı. Build + test yeşil.
