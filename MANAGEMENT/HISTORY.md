...
**Bug:** Gerçek cihaza `adb install -r` ile kurulum 2 kez `INSTALL_FAILED_USER_RESTRICTED` hatası verdi — Xiaomi/MIUI HyperOS'un fiziksel telefonda onay gerektiren bir kısıtlaması olabilir, adb'den çözülemedi, kullanıcıya Telegram'dan bildirildi (Ortam Sorunu Bildirim Kuralı).
**Sonraki:** Kullanıcının gerçek cihazda APK'yı manuel kurup fix'i doğrulaması bekleniyor.
