...
| 2026-07-14 | 15p | **🐛 "Sınıflandırılmamış: N" satırı tıklanınca açılmıyor** — Tamamlandı: `SettingsStatsScreen.kt` satırı `SettingsButtonRow` ile tıklanabilir yapıldı, `onNavigateToClassificationReview` ile `Routes.CLASSIFICATION_REVIEW`'a bağlandı (D269). | ✅ Tamamlandı |
| 2026-07-14 | 15p | **🐛 "Tüm istatistikleri sıfırla" 2 kez onay istemeli** — Tamamlandı: `SettingsAppsSection.kt`'deki "Tüm Kategorileri Sıfırla" minimal iki aşamalı onay akışına geçirildi (D269). Tam sihirbaz (veri seçimi ekranı) kapsam dışı bırakıldı, aşağıdaki "İstatistik sıfırlama sihirbazı" fikri ayrı kalır. | ✅ Tamamlandı |
...
| 2026-07-21 | 17p | **Performans iyileştirme (cold start/geçişler/cache/pil)** — HISTORY.md Döngü P23'te ölçülen %14.11 janky frame + Döngü 264'teki açık "cold-start jank" maddesi hiç genişletilmemişti; Baseline Profile, Compose stability, Room Flow distinctUntilChanged, splash screen eksikliği tespit edildi. Detaylı PERF-1..PERF-4 faz planı → ROADMAP.md "Faz PERF". (KV:5·U:4·BR:3·EA:5 = **17p**) | Bekliyor — PERF-1 (ücretsiz ölçüm) başladı |
---
...
|-------|--------|-------|-------|
| 2026-07-20 | Hüseyin talebi (rekabet parite) | **Serbest sürükle-bırak ana ekran sistemi** — widget/klasör/ikonların ana ekranda 2D serbest konuma sürüklenebilmesi (Nova/Lawnchair paritesi). Detaylı S1-S5 faz planı → ROADMAP.md "Faz S". (KV:5·U:2·BR:2·EA:5 = **14p**) | Bekliyor — büyük mimari iş, S1'den başlanmalı |
...
| 2026-07-14 | Hüseyin geri bildirim listesi (madde 16) | **🐛 "Sınıflandırılmamışları sınıflandır" Türkçe karakter hatası (encoding)** — Tamamlandı: `AppListViewModel.kt` çift/bozuk UTF-8 (mojibake) `scripts/fix_encoding.py` + elle düzeltildi; `SettingsAppsSection.kt`, `SettingsStatsScreen.kt`, `AppNavigation.kt`, `SettingsComponents.kt` em-dash/BOM sorunları da giderildi (D269). | ✅ Tamamlandı |
| 2026-07-14 | Hüseyin geri bildirim listesi (madde 17) | **🐛 Birleşik arama kapsamı eksik (kategori/klasör/dosya)** — D266'da doğrulandı: klasör adı (özel ad dahil) zaten aranıyor, kategori adı = klasör adı. Detay → ROADMAP.md [17]. | ✅ Tamamlandı (D266) |
