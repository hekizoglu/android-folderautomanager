...
| Tarih | Kaynak | Madde | Durum |
|-------|--------|-------|-------|
| 2026-07-09 | ThemePreferences.kt + OnboardingScreen.kt | **EN locale'de hardcoded Türkçe metinler kalıyor (Settings/Onboarding geneli)** — Döngü 218'de 16 eksik `values-en/strings.xml` çevirisi eklendi ve doğrulandı (onboarding artık büyük ölçüde EN), ama `ThemePreferences.kt`'deki tema/font enum `label`'ları ("Turkuaz", "Mor", "Sistem" vb., ~12 string) ve `OnboardingScreen.kt`'deki Quick Settings toggle başlık/alt yazıları (8 string) hâlâ Kotlin literal — `stringResource()` kullanmıyor. Muhtemelen Settings ekranlarının geneli de aynı sorunu taşıyor, tam kapsam çıkarılmadı. (KV:4 · U:3 · BR:3 · EA:4 = **14p**) | Bekliyor |
...
| 2026-07-09 | Döngü 221 — NotificationReportScreen kod incelemesi | **"Analiz kapalı" için ayrı UI mesajı yok** — `notif_analytics_enabled` kapalıyken rapor sessizce boş rapor durumuna düşüyor, kullanıcıya "neden boş" açıklaması yok; ayrı bir "analiz kapalı" empty-state metni eklenebilir. (KV:2·U:3·BR:2·EA:2) | 9p |
