...
| 2026-07-08 | 16p | **`BIND_ACCESSIBILITY_SERVICE` hiçbir yerde beyan edilmemişti** — `LauncherAccessibilityService.kt` manifestte kayıtlı (drag&drop için, şu an boş stub) ama `privacy_policy.html`/`store_listing.md`/ROADMAP/FİKİRLER'de hiç geçmiyordu. Google Play Accessibility API kullanımını Prominent Disclosure formuyla ayrıca denetler, en sık red sebeplerinden biri. (KV:4 · U:5 · BR:4 · EA:3 = **16p**) | ✅ Tamamlandı (Döngü 215) — privacy_policy.html §6'ya servisin ne yaptığı/yapmadığı (şu an no-op stub) net şekilde eklendi |
