...
| 2026-07-14 | 16p | **🐛 İzin ver butonu izin sonrası takılı kalıyor** — callback state güncellenmiyor. Detay → ROADMAP.md "Hüseyin Geri Bildirim Listesi" [1]. (KV:5·U:4·BR:4·EA:3) | ✅ Tamamlandı (Döngü 265) — `ContextualPermissionDialog.kt` checkSelfPermission çapraz doğrulama + ON_RESUME re-check |
| 2026-07-14 | 15p | **🐛 Silip tekrar kurunca onboarding başlamıyor** — Auto Backup şüphesi (`AppPrefs` restore). Detay → ROADMAP.md [2]. (KV:5·U:3·BR:4·EA:3) | ✅ Tamamlandı (Döngü 265) — cihaza-özel `install_marker` dosyası backup/device-transfer dışına alındı |
...
| 2026-07-14 | Hüseyin geri bildirim listesi (madde 6) | **Ticker swipe çalışmıyor** — gesture/nested scroll çakışması. Detay → ROADMAP.md [6]. (KV:4·U:4·BR:3·EA:3) | 14p — Bekliyor 🐛 |
