...
object MissionEngine {
    ...
    data class MissionCheckInput(
        ...
        val manuallyCompletedIds: Set<String> = emptySet(),
    )
    ...
    const val WEEKLY_REMOVE_UNUSED = "weekly_remove_unused"
    ...
    private val DAILY_POOL = listOf(
        ...
        Mission(DAILY_CUSTOMIZE_FOLDER, MissionType.DAILY, DAILY_STAR, autoCheckable = false),
        Mission(DAILY_VIEW_NOTIF_REPORT, MissionType.DAILY, DAILY_STAR, autoCheckable = false),
    )

    private val WEEKLY_POOL = listOf(
        Mission(WEEKLY_SCREEN_LESS, MissionType.WEEKLY, WEEKLY_STAR, autoCheckable = true),
        Mission(WEEKLY_REMOVE_UNUSED, MissionType.WEEKLY, WEEKLY_STAR, autoCheckable = false),
    )
    ...
    fun checkProgress(mission: Mission, input: MissionCheckInput): Boolean {
        if (mission.id in input.manuallyCompletedIds) return true
        return when (mission.id) {
            ...
            else -> false // manuel gorevler (klasor ozellestir, rapor incele, uygulama kaldir)
        }
    }
}
