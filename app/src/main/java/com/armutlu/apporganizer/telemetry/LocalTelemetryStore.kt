...
object LocalTelemetryStore {
    ...
    private val aggregateNames =
        mapOf("search_performed" to "search", "folder_opened" to "folder", "mission_completed" to "mission", "report_viewed" to "report")
    ...
    @Synchronized fun takeDailySummaries(context: Context, day: LocalDate = LocalDate.now()): Pair<TelemetryEvent, TelemetryEvent>? {
        ...
        val usage = TelemetryEvent.DailyUsageSummary(
            mapOf(
                ...
                "widget_active" to prefs.getBoolean("widget_active", false).toString(),
                "top_feature" to top,
            ),
        )
        val health = TelemetryEvent.DailyHealthSummary(
            mapOf(
